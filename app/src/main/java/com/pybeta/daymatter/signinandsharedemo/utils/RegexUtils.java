...
public class RegexUtils {
    ...
    private static final String PASS_TRUE = "^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z.,;:`~!@#$%^&*()_{}'+/-?]{6,12}$";

    //必须且只含有数字和字母，6-12位。
    private static final String PASS_TRUE_ONE = "^(?![0-9]+$)(?![a-zA-Z]+$)[0-9A-Za-z]{6,12}$";

    private static final String PASS_TRUE_TWO = "[\u4e00-\u9fa50-9a-zA-Z_]{6,12}";

    /**
     *验证
     * @param Pattern_String 正则验证内容
     * @param validateString 输入的内容
     * @return
     */
    public static boolean validate(String Pattern_String, String validateString) {
        try {
            // 如果规则和验证内容为null或为空就返回false;
            if (Pattern_String == null || Pattern_String.length() <= 0 || validateString == null) {
                return false;
            }
            return getValidator(Pattern_String).matcher(validateString).matches();
        } catch (Exception e) {
            return false;
        }
    }
    ...
    public boolean isPass(CharSequence temp){
        Pattern pattern = Pattern.compile(PASS_TRUE);
        Matcher matcher = pattern.matcher(temp);
        return matcher.matches();
    }

    /**
     * 检测输入的密码是否符合6到12位含有数字与字母要求
     * @param str 输入的密码
